...
class LurchLogger private (
  ...
  def logToTortureChamber(message: => String)(using Logger[IO]): OptionT[IO, Message] =
    log(tortureChamberChannelDeferred, message)

  def complete(
    discord: Discord,
    commonChannelConfiguration: ChannelConfiguration,
    channelConfiguration: LurchChannelConfiguration,
    ...
    for
      ...
      tortureChamberChannel <-
        DiscordLogger.getChannel(discord, "torture chamber", channelConfiguration.tortureChamberID)
