...
class TaskCompleter(
  ...
  private def handleTask(task: PendingTask, user: User)(using Logger[IO]): IO[Unit] =
    user.discordID match {
      ...
      case _ if !task.completed =>
        for
          _ <- task
                 .message(tortureChamberChannel)
                 .semiflatMap(_.removeUserReaction(pattern, task.keyholder).logErrorOption)
                 .value
  ...
  override def apply(pattern: String, event: ReactionEvent)(using Logger[IO]): IO[Boolean] =
    for
      ...
      maybeTask     = pendingTasks.find(task =>
                        task.user.discordID == event.author.discordID || task.keyholder.discordID == event.author.discordID,
                      )
