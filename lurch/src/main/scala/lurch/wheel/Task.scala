...
import bot.chaster.{ChasterClient, Lock, Segment}
...
import lurch.{Lurch, LurchLogger}
import lurch.commands.{TaskCompleter, Task as TaskCommand}
...
object Task:
  def handleTask(
    ...
    OptionT(fr"call GetTask(${user.discordID}, $task)".query[TaskCommand].option.transact(mySqlTransactor))
      ...
      .flatMap { task =>
        discordLogger.logToTortureChamber(s"${user.mention} rolled task ${task.id} - ${task.title}")
