...
class ChasterClient private(
  ...
  private def getAll[A <: WithID : Decoder](uri: Uri): TokenAuthenticatedStream[A] =
    Kleisli { (token: UserToken) =>
      ...
      for
        responses <- Stream.unfoldEval(client.expect[Result[A]](POST(WithLastID(), uri, authorization))
          .map(_ -> true)
        ) { result =>
          result.map {
            case (_, false) => None
  ...
  private def getAllBody[A <: WithID : Decoder, Body: Encoder](uri: Uri, body: Body): TokenAuthenticatedStream[A] =
    ...
    Kleisli { (token: UserToken) =>
      val authorization = token.authorizationHeader
      for
        responses <- Stream.unfoldEval(client.expect[Result[A]](POST(createBody(), uri, authorization))
          .map(_ -> true)
        ) { result =>
          result.map {
            case (_, false)     => None
  ...
  def keyholderLocks: TokenAuthenticatedStream[Lock] =
    Kleisli { (token: UserToken) =>
      ...
      for
        responses <- Stream.unfoldEval(client.expect[PagesResult[Lock]](POST(Paged(0), uri, authorization))
          .map(_ -> 0)
        ) { result =>
          result.map {
  ...
  def modifyTime(lock: ChasterID, modification: FiniteDuration): TokenAuthenticated[Unit] =
    expectAuthenticated[Unit](POST(
      Json.obj("duration" -> Json.fromLong(modification
        .toSeconds
      )
  ...
  def setFreeze(lock: ChasterID, freeze: Boolean): TokenAuthenticated[Unit] = expectAuthenticated(POST(Json.obj("isFrozen" -> Json
    .fromBoolean(freeze)), API / "locks" / lock / "freeze"))
  ...
  def action[E, Response: Decoder](
    ...
    expectAuthenticatedOrChasterError(POST(ExtensionAction(action, payload), API / "locks" / lock / "extensions" /
      extension / "action"))(recover)
  ...
  def updateExtensions(lockID: ChasterID)(update: List[ConfigUpdate[ExtensionConfig]] =>
    List[ConfigUpdate[ExtensionConfig]]): TokenAuthenticated[Unit] =
    for
      lock <- lock(lockID)
  ...
  def updateExtension[Config <: ExtensionConfig : Typeable](lockID: ChasterID)(update: ConfigUpdate[Config] =>
    ConfigUpdate[Config]): TokenAuthenticated[Unit] =
    for
      lock <- lock(lockID)
  ...
  def vote(lock: ChasterID, extension: ChasterID, action: VoteAction, sharedLink: ChasterID): TokenAuthenticatedEither[String, FiniteDuration] =
    this.action[String, VoteResponse](lock, extension)("vote", VotePayload(action, sharedLink)) {
