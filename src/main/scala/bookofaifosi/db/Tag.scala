...
object Tag extends Queries[Tag]:
  ...
  def add(name: String, description: Option[String]): ConnectionIO[Unit] =
    sql"insert into tags(name, description) values ($name, $description)".updateWithLogHandler(LogHandler.jdkLogHandler).run.void

  def remove(name: String): ConnectionIO[Int] =
    sql"delete from tags where name = $name".updateWithLogHandler(LogHandler.jdkLogHandler).run
