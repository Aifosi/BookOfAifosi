...
object SlashPattern:
  ...
  def buildCommands(commands: List[SlashPattern]): List[SlashCommandData] =
    commands
      .groupBy(_.name)
      .view
      .mapValues(_.groupBy(_.subCommandGroup).view.mapValues(_.groupBy(_.subCommand)).toMap)
      .toList
      .map { case (commandName, other) =>
            ...
            case (command, (None, other))                                              =>
              other.foldLeft(command) {
                ...
                case (command, (Some(subCommandName), List(pattern))) =>
                  ...
                  val subCommand =
                    new SubcommandData(subCommandName, pattern.description).applyOptions(pattern.subCommandOptions)
