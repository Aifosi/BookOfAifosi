...
import bot.model.{Channel, Discord, DiscordID, Message}
...
open class DiscordLogger protected (
  ...
  def logToChannelStream(message: => String)(using Logger[IO]): Stream[IO, Nothing] =
    logStream(logChannelDeferred, message)
  def logToSpinlogStream(message: => String)(using Logger[IO]): Stream[IO, Nothing] =
    logStream(spinlogChannelDeferred, message)
