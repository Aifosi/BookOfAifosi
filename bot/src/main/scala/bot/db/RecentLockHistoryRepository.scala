...
class RecentLockHistoryRepository(
  ...
  override def toModel(lockHistory: RecentLockHistory): Maybe[RecentLockHistoryModel] =
    for user <- registeredUserRepository.get(lockHistory.userID.equalID).to[Maybe]
  ...
  def add(
    ...
    sql"insert into $table (user_id, lock_id, most_recent_event_time) values ($userID, $lockID, $mostRecentEventTime)".update
