...
object DoobieLogHandler:
  private def createLogHandler(using Logger[IO], IORuntime): LogHandler = LogHandler {
    case Success(s, a, e1, e2) =>
      Logger[IO]
        .debug(
          s"""Successful Statement Execution: ${s.linesIterator.dropWhile(_.trim.isEmpty).mkString("\n  ")}
                        | arguments = [${a.mkString(", ")}]
                        |   elapsed = ${e1.toMillis.toString} ms exec + ${e2.toMillis.toString} ms processing (${(e1 + e2).toMillis.toString} ms total)""".stripMargin,
        )
        .unsafeRunSync()

    case ProcessingFailure(s, a, e1, e2, t) =>
      Logger[IO]
        .error(s"""Failed Resultset Processing: ${s.linesIterator.dropWhile(_.trim.isEmpty).mkString("\n  ")}
                          | arguments = [${a.mkString(", ")}]
    ...
    case ExecFailure(s, a, e1, t) =>
      Logger[IO]
        .error(s"""Failed Statement Execution: ${s.linesIterator.dropWhile(_.trim.isEmpty).mkString("\n  ")}
                          | arguments = [${a.mkString(", ")}]
